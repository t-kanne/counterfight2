...
public class MainActivity extends FragmentActivity implements FragmentSwitcher {
	...
	ArrayList<DrawerItem[]> childItems = new ArrayList<DrawerItem[]>();	// Array-List f�r alle Gruppenelemente
	ArrayList<String> expandGroupsChild;	// Array-List f�r DYNNAMISCHE GRUPPEN
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if (sessionManager.isLoggedIn() == false) {					// Login-Status des Nutzers �berpr�fen.
			...
			new CountUserGroups().execute();						// Gruppen des Users z�hlen, um ihn zur entsprechenden Activity weiterzuleiten
		}
				
		
		// ***************************************************************************************
		// AB HIER: ClickListener f�r den NavigationDrawer
		// ***************************************************************************************
		
		// Der OnGroupClickListener sorgt nur daf�r, dass die Gruppen nicht einklappbar sind. Sie f�hren zu keinem Fragment
		expListView.setOnGroupClickListener(new OnGroupClickListener() {
			...
			public boolean onGroupClick(ExpandableListView parent, View v, int groupPosition, long id) { 
				if (getIndividualGroupsOfUser().size() > 0 && groupPosition == 1) {		// Nur Gruppe 1 (Einzelgruppen) kann eingeklappt werden
					...
					return true; // Gruppen k�nnen nicht eingeklappt werden
				}	
			}
		});	
		...
		expListView.setOnChildClickListener(new OnChildClickListener() {
			...
			public boolean onChildClick(ExpandableListView parent, View v,
					...
					if (groupPosition == GROUP_POS_ALLGROUPS) {
							Toast.makeText(MainActivity.this, "Funktion noch nicht verf�gbar", Toast.LENGTH_SHORT).show();
					}
	...
	public void initializeDrawer(){
		...
		// Gruppen standardm��ig ausklappen
	...
	// ######### WICHTIG #############
	// Die Reihenfolge der folgende Methoden-Abschnitte muss zwingend so eingehalten werden. Wenn die Men�punkte verschoben werden sollen,
	...
	public void setGroupData() {
		// Gruppenverwaltung (0)
		DrawerItem groupSettingsChild = new DrawerItem();													// DrawerItem-Object f�r Gruppenverwaltung
		groupSettingsChild.setTitle(getString(R.string.string_navigationdrawer_groupsettings));				// Titel
		groupSettingsChild.setLayoutType(LAYOUT_SECTION_TITLE);												// Section-Layout enth�lt kein Icon
		...
		if (getIndividualGroupsOfUser().size() > 0) {														// "Gruppen aufklappen" nur zeigen, wenn vorhanden
			...
			expandGroupData.setLayoutType(LAYOUT_TEXT_ONLY);													// Layout // LAYOUT_TEXT_ONLY enth�lt kein Icon, daher null bei KEY_ICON
			expandGroupData.setExtras(R.string.string_navigationdrawer_collapsegroups);							// Extra: zus�tzlicher Titel bei ausgeklapptem Men�
	...
	public void setChildGroupData() {
		// Gruppenverwaltung
		DrawerItem[] groupSettingsChildren = new DrawerItem[3]; 													// Array f�r alle Elemente der Gruppenverwaltung		
		DrawerItem groupSummaryChild = new DrawerItem();										// DrawerItem f�r Gruppen�bersicht-Element		
		...
		// -------------------------------------------------------------------------------------------------------------------------------------------
		// Gruppen ausklappen	- SONDERFALL: f�r jedes Element wird hier dasselbe Icon verwendet
		if (getIndividualGroupsOfUser().size() > 0) {
			DrawerItem[] individualUserGroupChildren = new DrawerItem[getIndividualGroupsOfUser().size()]; // Array-List f�r alle Usergruppen
			...
			for (int i = 0; this.getIndividualGroupsOfUser().size() > i; i++) {
				individualUserGroup = new DrawerItem();													// DrawerItem-Object f�r pers�nliche Gruppe
	...
	protected void onResume() {
		...
		Log.d("MainActivity", "onResume() ausgef�hrt");
	}
	
	
	// �berschriebene Methode vom Interface "FragmentSwitcher"
	// Wird von CreateGroupDialog() und SearchGroupDialog() aufgerufen, um Weiterleitung auf das GroupDetailFragment zu erm�glichen
	@Override
